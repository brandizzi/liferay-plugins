				...
				<c:if test="<%= themeDisplay.isSignedIn() %>">
					<div class="toggler-header-expanded calendar-portlet-list-header">
						...
						<c:if test="<%= userCalendarResource != null %>">
							<span class="calendar-list-item-arrow" data-calendarResourceId="<%= userCalendarResource.getCalendarResourceId() %>" data-prefix="my" tabindex="0"><i class="icon-caret-down"></i></span>
						</c:if>
					</div>
				...
				<c:if test="<%= groupCalendarResource != null %>">
					<div class="toggler-header-expanded calendar-portlet-list-header">
						...
						<c:if test="<%= CalendarResourcePermission.contains(permissionChecker, groupCalendarResource, ActionKeys.ADD_CALENDAR) %>">
							<span class="calendar-list-item-arrow" data-calendarResourceId="<%= groupCalendarResource.getCalendarResourceId() %>" data-prefix="site" tabindex="0"><i class="icon-caret-down"></i></span>
						</c:if>
					</div>
...
<aui:script use="aui-toggler,liferay-calendar-list,liferay-scheduler,liferay-store,json">
	...
	Liferay.CalendarUtil.RESOURCE_CALENDARS_URL = '<liferay-portlet:resourceURL copyCurrentRenderParameters="<%= false %>" id="resourceCalendars"><liferay-portlet:param name="calendarResourceId" value="{calendarResourceId}" /></liferay-portlet:resourceURL>';
